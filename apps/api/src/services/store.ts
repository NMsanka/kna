...
import { withSystemContext, withOrgContext, type DbHandle } from '@kna/db';
...
export class PlatformStore {
  ...
  async principalForToken(
    ...
    >`
      SELECT p.id, p.org_id, p.subject, p.email, p.clearance, p.is_service_account, p.disabled_at
      FROM api_tokens t
  ...
  async reposForProjects(orgId: string, projectIds: string[]): Promise<string[]> {
    ...
    const rows = await withOrgContext(this.db, orgId, async (tx) =>
      tx.execute<{ repo_id: string }>(sql`
        ...
        WHERE m.org_id = ${orgId} AND mp.project_id = ANY(${projectIds})
      `),
    );
      ...
      tx.execute<{ project_id: string }>(sql`
        ...
        WHERE e.org_id = ${orgId}
          ...
          AND e.from_symbol_id IN (
            ...
            WHERE mp.project_id = ANY(${projectIds})
          )
        LIMIT 20
      `),
    );
